...
General information
-------------------
 These modules are built with JavaScript and jQuery, they are modestly styled with Twitter's Bootstrap framework and we user JSRender as the template engine. To render graphs and charts we use Highstock. Highstock is needed for the sharegraph while the other modules can use the more lightweight Highcharts. 

 Each module has its own JavaScript file which we compile into a single websolutions script. If there is any need to load them separately or any changes are required for specific implementation reasons or preferences all the scripts can be downloaded separately from [our demosite](http://websolutions.ne.cision.com/code). If changes are made to our scripts Cision can no longer guarantee support if any problem arises. 
...
```javascript
var formattedNumber = cision.websolution.formatHelpers.formatNumber(value, decimalPrecision, decimalSeparator, thousandSeparator)
```
...
### Subscription
Our subscription module does not have an example since it is a simple form that posts to the follopwing endpoint `https://publish.ne.cision.com/Subscription/Subscribe`.
...
### Moment JS
By Tim Wood, this is used to parse and format date and time. We use moment with locales for supporting different formats depending on culture.
...
### Bootstrap 
By Twitter, Inc, this is used to make the layout design smoother and the site responsive.
...
### Underscore
Open source javascript utility library used by most modules for mapping, sorting, filtering of arrays among other things.
